...
namespace Anh.BeginUnicode
{
    ...
    {
        public async static Task<UniCodeRange[]> Main()
        {
            UniCodeRange[] x = await ReadXml();
            return x;
        }

        private async static Task<UniCodeRange[]> ReadXml()
        {
    ...
    {
        public async static Task<UnicodeData[]> Main(params object[] pr)
        {
            if ((bool)pr[1])
            {
                UnicodeData[] x = await ReadXml((string)pr[0]);
                return x;
            }
            else
            {
                UnicodeData[] x = await ReadXmlFile((string)pr[0]);
                return x;
            }
        }
        ...
        public async static Task SaveXml(UnicodeData[] list, string fileName)
        {
            Task t = Task.Run(() =>
            {
    ...
    {
        public async static Task<string> Code(string Url)
        {
